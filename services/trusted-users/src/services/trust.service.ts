...
export class TrustService {
  ...
  async addTrusted(did: string) {
    try {
      ...
      if (
        ...
        error.message === "Trust relationship already exists"
      ) {
  ...
  async removeTrusted(did: string) {
    try {
      ...
      if (
        error instanceof Error &&
        error.message === "Trust relationship does not exist"
      ) {
  ...
  private async removeTrustedUser(
    authorDid: string,
    recipientDid: string,
  ): Promise<void> {
    await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
  ...
  private async getTrustedRecipients(
    authorDid: string,
  ): Promise<Array<{ did: string; createdAt: Date }>> {
