...
describe('Testimonials API Tests', () => {
  ...
  describe('createTestimonial endpoint', () => {
    ...
    it('should store facets when provided', async () => {
      ...
      expect((testimonial?.content as { facets?: unknown[] }).facets).toEqual(
        facets,
      );
    });
  });
  ...
  describe('listTestimonials endpoint', () => {
    ...
    it('should return testimonials ordered by createdAt descending', async () => {
      ...
      expect(
        (response.body.testimonials[0].content as { text: string }).text,
      ).toBe('Second');
      expect(
        (response.body.testimonials[1].content as { text: string }).text,
      ).toBe('First');
    });
    ...
    it('should filter by did', async () => {
      ...
      await prisma.testimonial.create({
        data: {
          did: anotherContributorDid,
          content: { text: 'From contributor 2' },
        },
      });
  ...
  describe('updateTestimonial endpoint', () => {
    ...
    it('should update facets', async () => {
      ...
      const facets = [
        {
          index: { byteStart: 0, byteEnd: 5 },
          features: [{ $type: 'app.bsky.richtext.facet#tag', tag: 'updated' }],
        },
      ];
      const response = await request(server.express)
        .post('/xrpc/social.spkeasy.actor.updateTestimonial')
        .send({
          id: testimonial.id,
          content: { text: '#updated post', facets },
        })
        .set('Authorization', `Bearer ${contributorToken}`)
        .expect(200);
      ...
      expect(
        (updated?.content as { text: string; facets?: unknown[] }).text,
      ).toBe('#updated post');
      expect(
        (updated?.content as { text: string; facets?: unknown[] }).facets,
      ).toEqual(facets);
    });
  });
